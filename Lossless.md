...
### 5.1 反向 alpha 后端 (已实现 — v0.3.0)

```python
# src/watermark/services/alpha.py (实际代码)
...
```python
# calibrate_from_solid_video — 从纯色背景视频标定 alpha
alpha, meta = alpha_svc.calibrate_from_solid_video(
    Path("grey.mp4"),
    background_rgb=(126, 126, 127),  # grey.mp4 实测背景色
    logo_rgb=(255, 255, 255),
    alpha_size=96,
)
# → alpha: (96, 96) float32, meta: {bbox, background_rgb, source, ...}
```

CLI:
```bash
uv run watermark calibrate --background-rgb 126,126,127 --video grey.mp4
```
...
**文档版本**: v0.2.0 (2026-06-04)
**状态**: 已实现。`src/watermark/services/alpha.py` + `--backend alpha` + PNG 层验证。
