...
@pytest.fixture
def fake_remove_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> RemoveEnv:
...
def test_remove_cmd_runs_full_pipeline(fake_remove_env: RemoveEnv) -> None:
    """`watermark remove` orchestrates detect -> mask -> inpaint -> reassemble."""
    ...
    assert calls.mask, "mask not called"
